...
@ccclass
export default class BCBMPlayerMgr extends PlyMgr<BCBMPlayer, BCBMGame> {
    ...
    setBigRegalGambleGodPos() {
        ...
        if (!people || people.length < 1) {
            setTimeout(() => { this.setBigRegalGambleGodPos() }, 200);
            return;
        }
        people.sort((a, b) => {
     ...
     * @param player 富豪或赌神
     */
    private chgBigRegalGambleGod(newPos: number, player: BCBMPlayer) {
        // 填补新富豪
    ...
    updatePly(p: BCBMPlayer, data: ps.User) {
