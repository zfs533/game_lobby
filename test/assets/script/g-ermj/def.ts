...
export const quanFengTile = {
    [quanFeng.east]: MJ_TYPE.MJ_TYPE_FE,
    [quanFeng.south]: MJ_TYPE.MJ_TYPE_FS,
    [quanFeng.west]: MJ_TYPE.MJ_TYPE_FW,
    [quanFeng.north]: MJ_TYPE.MJ_TYPE_FN
}

export const menFengTile = {
    [menFeng.south]: MJ_TYPE.MJ_TYPE_FS,
    [menFeng.north]: MJ_TYPE.MJ_TYPE_FN
}
...
export const waitTime = 2           // 等待设置时长
export const sendCardTime = 2       // 发牌时间
export const userOptTime = 15        // 玩家操作时间
export const resultTime = 2
...
export class PingHuStruct {
    public pingHuData: { byType: number, tile: number, chiPai?: number }[]
    constructor() {
        this.Init()
    }
    ...
    DeleteData = (type: number, pai: number) => {
        for (let i = 0; i < this.pingHuData.length; i++) {
            if (this.pingHuData[i].byType === type && pai == this.pingHuData[i].tile) {
