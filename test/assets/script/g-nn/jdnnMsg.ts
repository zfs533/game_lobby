...
import JdnnGame, { GameState } from "./jdnnGame"
...
export default class JdnnMsg extends GameMsg {
    ...
    protected regMsgHanlder(): void {
        // cc.log('reg...........')
        this.onMsg(ROUTE.jdnn_StatusChange, this.phaseChange)
        this.onMsg(ROUTE.jdnn_DoDealer, this.handleGrabDealer)
        this.onMsg(ROUTE.jdnn_AppointDealer, this.handleShowDealer)
        this.onMsg(ROUTE.jdnn_DoBet, this.handleBet)
        this.onMsg(ROUTE.jdnn_DealCards, this.handleDealCards)
        this.onMsg(ROUTE.jdnn_DoHandout, this.handleDoHandout)
        this.onMsg(ROUTE.jdnn_EnterBalance, this.handleResult)
    }
    ...
    private handleGrabDealer(data: ps.Jdnn_DoDealer) {
        // cc.log("-----handleGrabDealer-----", data);
    ...
    private handleShowDealer(data: ps.Jdnn_AppointDealer) {
    ...
    private handleBet(data: ps.Jdnn_DoBet) {
        let p = this.getPlayer(data.pos)
        if (!p) {
            console.warn("no body grab dealer")
            return
    ...
    private handleDealCards(data: ps.Jdnn_DealCards) {
        this.phaseChange({ status: GameState.STATUS_DEAL_CARD, timer: 0 })
        this.dealSomeCards(data.cards)
    }
    private handleDoHandout(data: ps.Jdnn_DoHandout) {
    ...
    private phaseChange(data: ps.Jdnn_StatusChange, isReconnect = false) {
        let game = this.game
        ...
        if (data.status < GameState.STATUS_BALANCE) {
            if (me.isLooker && this._showForLooker) {
                ...
                if (data.status >= GameState.STATUS_DEAL_CARD)
                    this.dealSomeCards([0, 0, 0, 0, 0], data.status === GameState.STATUS_DEAL_CARD)
            }
        }
        ...
        switch (data.status) {
            ...
            case GameState.STATUS_DEAL_CARD:
                //隐藏时钟
                game.hideTicker()
    ...
    private async handleResult(data: ps.Jdnn_EnterBalance) {
    ...
    protected handleGameInfo(data: ps.Jdnn_GameInfo) {
        if (!data) return
    ...
    sendBet(val: number) {
        net.notify("game.jdnnHandler.doBet", { betPoint: val })
    }

    sendDealer(val: number) {
        net.notify("game.jdnnHandler.doDealer", { dealerPoint: val })
    }

    //计算完成
    sendFinish() {
        net.notify("game.jdnnHandler.doHandout", {})
    }
}